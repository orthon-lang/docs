...
| `LLM_GENERABILITY_GATE` | Is the concept LLM-generable — can an LLM reliably produce correct code using this construct? | [Empirical analysis](methods/EMPIRICAL_ANALYSIS_METHOD.md) — structural analysis + schema round-trip |
...
| Syntax change | `LOGICAL_CONSISTENCY_GATE`, `CONCEPTUAL_SIMPLICITY_GATE`, `ARCHITECTURAL_INTEGRITY_GATE`, `LLM_GENERABILITY_GATE` | `USER_VALUE_GATE` (if purely syntactic sugar), `IMPLEMENTATION_INDEPENDENCE_GATE`, `LONG_TERM_MAINTAINABILITY_GATE` |
| LLM Toolchain component | `ARCHITECTURAL_INTEGRITY_GATE`, `LLM_GENERABILITY_GATE`, `USER_VALUE_GATE` | `CONCEPTUAL_SIMPLICITY_GATE`, `LOGICAL_CONSISTENCY_GATE`, `IMPLEMENTATION_INDEPENDENCE_GATE`, `LONG_TERM_MAINTAINABILITY_GATE` |
| Semantic refinement | `LOGICAL_CONSISTENCY_GATE`, `ARCHITECTURAL_INTEGRITY_GATE` | `CONCEPTUAL_SIMPLICITY_GATE` (if narrowing existing concept), `LLM_GENERABILITY_GATE` |
| Standard Library addition | `USER_VALUE_GATE`, `CONCEPTUAL_SIMPLICITY_GATE`, `LONG_TERM_MAINTAINABILITY_GATE` | `ARCHITECTURAL_INTEGRITY_GATE` (if within Library), `IMPLEMENTATION_INDEPENDENCE_GATE`, `LLM_GENERABILITY_GATE` |
...
language rule is **all seven gates**.
...
**Perspective:** *Is the concept LLM-generable — can an LLM reliably
produce correct code using this construct?*

Orthon is designed for the LLM era (see `../../why/VISION.md`).
This gate ensures that every language construct can be reliably
generated by LLMs, with predictable syntax, unambiguous semantics,
and a machine-readable schema that eliminates hallucination.

**Method:** Empirical analysis with two complementary approaches:

1. **Structural analysis** — examine the concept's grammar and
   semantics for known LLM-unfriendly patterns (ambiguity, special
   cases, context-dependent syntax, tokenizer misalignment).
...
**Fail conditions:**
- The concept cannot be expressed in a machine-readable grammar
  schema without losing essential constraints.
- An LLM consuming only the canonical schema would systematically
  generate syntactically or semantically invalid code.
- The concept introduces ambiguity that different LLM runs resolve
  differently.
...
**See also:** `EDR-014-llm-generability-gate`, `../../how/concepts/research/LLM_NATIVE_TOOLCHAIN.md`,
...
| Analytical | [Empirical Analysis](methods/EMPIRICAL_ANALYSIS_METHOD.md) | `LLM_GENERABILITY_GATE` | Constructs that LLMs cannot generate reliably |
...
| 2026-08-06 | [Execution Context Invocation](../../how/concepts/research/essential/EXECUTION_CONTEXT_INVOCATION.md) ([EDR-085](../decision_records/architecture/EDR-085-execution-context-invocation.md)) | All 7 | 4 Pass, 3 Flag | Flags: constructor surface (Level 2), two-operator family debt, distribution glyph deferred to Phase 5. `execution_context` added as 10th primitive. Full per-gate reasoning: [`DECISION_LOG.md` § EXECUTION_CONTEXT_INVOCATION](DECISION_LOG.md#entry-execution-context-invocation). |
